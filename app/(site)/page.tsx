  ...
  return (
    <>
      ...
      <section className="wrap min-h-[calc(100vh-66px)] flex flex-col justify-center py-16 relative">
        <Reveal className="kicker tracking-[0.28em] uppercase text-[11px] mb-[30px]">A living archive · est. 2026</Reveal>
        <Reveal as="h1">
          <span className="font-serif font-light text-[clamp(52px,10vw,124px)] leading-[0.98] tracking-[-0.005em] block">MUTAZOLOGY</span>
        </Reveal>
      ...
      {/* FEATURED */}
      <section className="wrap py-[56px] md:py-[88px] border-t border-line-soft">
        ...
        <Reveal>
          <Link href={`/thoughts/${featured.slug}`} className="block relative border border-line p-[clamp(34px,6vw,72px)] bg-gradient-to-b from-bg-soft to-bg overflow-hidden">
            <div className="absolute top-3 right-5 font-serif text-[110px] text-line leading-none select-none pointer-events-none z-0" aria-hidden="true">&ldquo;</div>
            <div className="relative z-10 font-serif font-light text-[clamp(24px,3.4vw,40px)] leading-[1.42] tracking-[-0.01em]"
              dangerouslySetInnerHTML={{ __html: featured.body.join("<br/>").replace(/\n/g, "<br/>") }} />
            <div className="relative z-10 mt-9 flex gap-5 items-center text-ink-faint text-xs tracking-[0.06em]">
...
function Stat({ n, l }: { n: number; l: string }) {
  return (
    <div className="bg-bg px-[26px] py-[38px]" style={{ padding: "38px 26px" }}>
      <div className="font-serif font-light text-[clamp(36px,5vw,58px)] leading-none tracking-[-0.01em]">{n}</div>
